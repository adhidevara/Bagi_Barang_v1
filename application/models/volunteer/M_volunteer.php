 ...
 class M_volunteer extends CI_Model {
	...
	public function selCampaignSelesaiAll($where){
 	...
 	public function selCampaignSelesai($where)
 	{
		...
		$this->db->where('ROUND((TIMESTAMPDIFF(DAY, tanggal_campaign, batas_campaign) - TIMESTAMPDIFF(DAY, NOW(), batas_campaign)) / TIMESTAMPDIFF(DAY, tanggal_campaign, batas_campaign)*100, 2)');
		$this->db->where('id_volunteer', $where);
	...
	public function searchCampaign($where, $val)
	{
		...
		$this->db->where('ROUND(TIMESTAMPDIFF(DAY, NOW(), batas_campaign) / TIMESTAMPDIFF(DAY, tanggal_campaign, batas_campaign)*100, 0) < 100');
		$this->db->where('id_volunteer', $where);
		$this->db->where('judul_campaign', $val);
		$this->db->order_by('id_campaign', 'desc');
